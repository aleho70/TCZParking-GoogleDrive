...
//          backeckend use main.py version 3
...
var GAPlugin = function(){
	...
	return {
      ...
      if(_gaq) _gaq.push(['_setCustomVar',
...
//	var PARKING_BACKEND_URL = 'http://localhost:8080/getvalue';
	var PARKING_BACKEND_URL = 'https://to2parking.appspot.com/getvalue';
	var parkingCount = 0; 
      ...
      var nameElement = (enableDetail) ? 'h3' : 'b';
            ...
            //(!enableDetail) 'data-mini="true"' : '' +
				...
				if(enableDetail && value.status) {
					items.push(
							'<p>'+value.address+'</p>'+
							'<p>Čas aktualizace: <b>'+(value.status[0]==undefined?'?':value.status[0])+'</b></p>'+
							'<span class="ui-li-count">'+(value.status[2]==undefined?'?':value.status[2])+'</span>'+
							'<span class="ui-li-count">'+(value.status[1]==undefined?'?':value.status[1])+'</span>'
					);
				}
				items.push(
						'</a>'+
...
;							Debug.error('Not found parkingCode('+parkingCode+')');
							...
							$statusLastUpdate.text(status[0]==undefined?'?':status[0]);
...
//							console.log(new Date(Date.fromString(status[0], {order: 'DMY'})));
							...
							// Corporate
							var countCorporate = status[1]==undefined?'?':status[1];
							...
							// Private
							var countPrivate = status[2]==undefined?'?':status[2];
